...
func (h *Handler) PostApiShortenBatchHandler(w http.ResponseWriter, r *http.Request) {
	...
	var batchRecords []storage.ShortenBatch
	for _, batchRequest := range batchRequests {
		batchRecords = append(batchRecords, storage.ShortenBatch{
	...
	batchRecords, err = h.Storage.ApiShortenBatch(r.Context(), batchRecords)
	if err != nil {
		http.Error(w, err.Error(), 500)
		return
	}
